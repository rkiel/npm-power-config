...
function search(searchFor) {
  return _.reduce(
    searchFor,
    (a, e) => (a !== false ? a : fs.existsSync(e) ? e : false),
    false
  );
}
...
function parse(pathToFile, defaultValue) {
  if (fs.existsSync(pathToFile)) {
    ...
    if (pathToFile.includes(".json")) {
      ...
      x = yaml.safeLoad(contents);
    }
    if (_.isObject(x) && !_.isArray(x)) {
      return _.omit(x, ["UNUSED", "SKIP", "IGNORE", "CONSTANTS"]); // TODO: add to environments
    } else {
      return x;
    }
...
function write(pathToFile, json) {
  ...
  if (pathToFile.includes(".json")) {
    ...
    fs.writeFileSync(pathToFile, data, "utf8");
  } else if (pathToFile.includes(".ini")) {
    const items = _.map(_.keys(x), key => `${key}=${x[key]}`);
