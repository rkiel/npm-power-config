...
function replicateObjectByEnvironment(powerObject, namespace) {
  const powerObjects = [];
...
function convertToPowerObjects(state, namespace, object) {
  const powerObject = lib.expandToCompletePowerObject(object, namespace);
  return lib.replicateObjectByEnvironment(powerObject, namespace);
}

function processScalarValue(state, namespace, value) {
  return lib.convertToPowerObjects(state, namespace, lib.scalarToPowerObject(value));
}

function expandToCompletePowerObject(powerObject, namespace) {
  return _.assign(
    {},
    lib.defaults(),
    lib.namespace(namespace),
    lib.input(powerObject),
    lib.scalarToPowerObject(lib.valueOf(powerObject)),
    powerObject
  );
}
...
function scalarToPowerObject(value) {
