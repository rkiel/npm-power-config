...
function readFile(state) {
  ...
  if (fileName) {
    _.set(state, 'rc.settings', lib._file.parse(fileName));
  } else {
    _.set(state, 'rc.settings', {});
  }
  return state;
}

function addEnvironments(state) {
  _.set(state, 'environments.defined', _.get(state, 'rc.settings.environments', ['local', 'dev', 'test', 'prod']));
  return state;
}
