...
function processDataFieldObject(state, namespace, dataFieldObject, key) {
  if (lib._environment.shouldIgnore(state, dataFieldObject)) {
    return state;
  } else {
    let fieldValue;
    const fieldNamespace = _.concat(namespace, key);

    if (lib.isHardCodedValueObject(dataFieldObject)) {
      fieldValue = lib.extractHardCodedValue(dataFieldObject);
    } else if (lib.isIncludeFileObject(dataFieldObject)) {
      fieldValue = lib.extractIncludeFile(dataFieldObject);
    } else if (lib.isThereAPreExistingValue(state, fieldNamespace)) {
      fieldValue = lib.extractPreExistingValue(state, fieldNamespace);
    } else {
      fieldValue = lib.promptUserForValue(state, fieldNamespace, dataFieldObject);
    }
    lib.updateOuput(state, fieldNamespace, fieldValue);
    lib.updateInput(state, fieldNamespace, fieldValue);
    return state;
  }
}
